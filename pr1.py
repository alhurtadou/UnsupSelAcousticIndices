...
def computeIndices(fileList):
    ...
    for filename in fileList:
        ...
        try:
          ...
          # Output Indices -----------------------------------------------------------------------------------
          print('- Write Indices')
          writer = csv.writer(open('dict1.csv', 'a'))
...
def main():
    with open('dict1.csv', 'w') as f_object:
       f_object.close()
    fileList = []

    dir_path = r'/proyectoPTA/data/YATAROS/YAT1Audible/'
    for path in os.listdir(dir_path):
        if os.path.isfile(os.path.join(dir_path, path)):
           fileList.append('/proyectoPTA/data/YATAROS/YAT1Audible/'+path)
    #print(fileList)
    ...
    m = len ( fileList )// P
    print('particiones=', m)
    arg = [(fileList[ p * m :( p +1)* m ], None) for p in range (P -1)]
