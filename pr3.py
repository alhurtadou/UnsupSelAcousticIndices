...
def computeIndices(fileList):
    ...
    for filename in fileList:
        ...
        try:
          ...
          # Output Indices -----------------------------------------------------------------------------------
          print('- Write Indices')
          writer = csv.writer(open('dict3.csv', 'a'))
...
def main():
    with open('dict3.csv', 'w') as f_object:
       f_object.close()
    fileList = []

    dir_path = r'/proyectoPTA/data/YATAROS/YAT3Audible/'
    for path in os.listdir(dir_path):
        if os.path.isfile(os.path.join(dir_path, path)):
           fileList.append('/proyectoPTA/data/YATAROS/YAT3Audible/'+path)
    #print(fileList)
